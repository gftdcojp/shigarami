...
export class DepCompatMCPServer {
            ...
            try {
                switch (name) {
                    case 'search_compatibility':
                        return await this.handleSearchCompatibility(args);
                    case 'resolve_compatibility':
                        return await this.handleResolveCompatibility(args);
                    case 'get_compatibility_issues':
                        return await this.handleGetCompatibilityIssues(args);
     ...
     */
    async handleSearchCompatibility(args) {
        const query = {
            ...
            content: [
                {
                    ...
                    text: `Found ${issues.length} compatibility issues:\n\n` +
                        formattedResults.map(issue => `**${issue.framework}** - ${issue.status.toUpperCase()}\n` +
     ...
     */
    async handleResolveCompatibility(args) {
     ...
     */
    async handleGetCompatibilityIssues(args) {
        const issues = await Promise.all(args.ids.map(id => this.db.getIssue(id)));
        ...
        const detailedInfo = validIssues.map(issue => {
            ...
            if (issue.react)
                info += `**React:** ${issue.react}\n`;
            if (issue.node)
                info += `**Node.js:** ${issue.node}\n`;
            if (issue.packageManager)
                info += `**Package Manager:** ${issue.packageManager}\n`;
     ...
     */
    async handleGetCompatibilityStats() {
