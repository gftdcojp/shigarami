...
import type { CompatibilityIssue, CompatibilityQuery } from '../types/compatibility.js';
...
export class CLI {
   ...
   */
  async resolveDependencies(options: CLIOptions): Promise<void> {
    const projectRoot = options.projectRoot || process.cwd();
   ...
   */
  async checkCompatibility(framework: string, packages: string[], options: CLIOptions): Promise<void> {
    ...
    const additionalPackages: Record<string, string> = {};
    ...
    if (options.react) query.react = options.react;
   ...
   */
  async searchCompatibility(_query: string | undefined, options: CLIOptions): Promise<void> {
    const searchQuery: CompatibilityQuery = {
      limit: parseInt(options.limit || '10'),
    };

    if (options.framework) searchQuery.framework = options.framework;
   ...
   */
  async reportIssue(options: CLIOptions): Promise<void> {
   ...
   */
  async exportDatabase(outputFile: string): Promise<void> {
    try {
