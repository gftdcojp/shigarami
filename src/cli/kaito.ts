...
export const kaitoRunCommand = (options: any): Effect.Effect<void, Error> =>
  Effect.gen(function* (_) {
    ...
    if (options.config) {
        ...
        config = {
            ...
            environment: { node: '20.x', packageManager: 'npm@10.x' },
            dependencies: {
    ...
    const installOutput = yield* _(Effect.tryPromise(() => runCommand('npm', ['install'])));
    yield* _(Console.log(`   Running script: ${config.script}...`));

    let buildOutput = { stdout: "", stderr: "", exitCode: 0 };
    if (installOutput.exitCode === 0) {
      buildOutput = yield* _(Effect.tryPromise(() => runCommand('npm', ['run', config.script])));
    } else {
...
export const kaitoReportCommand = (options: { experimentHash: string }): Effect.Effect<void, Error> =>
  Effect.gen(function* (_) {
    const shigramiDir = path.join(os.homedir(), '.shigrami');
    ...
    yield* _(Console.log(`\nSubmitting report to shigrami database...`));
    yield* _(reportIssueCommand(reportOptions));
  });

// --- Helpers ---

const parsePkg = (pkg: string): { name: string; version: string } => {
