...
import { PropertyGraph, PropertyVertex, PropertyIncident, PropertyEvent, PropertyGraphConverter } from '../types/compatibility-vie.js';
import { NixStoreManager, CompatibilityDerivation, DerivationResult, DerivationBuilder } from './derivation.js';
...
export class PropertyGraphStoreManager {
  constructor(private storeManager: NixStoreManager) {}

  /**
   * Store a property graph based on derivation
   */
  async storePropertyGraph(
    derivation: CompatibilityDerivation,
   ...
   */
  async getPropertyGraph(derivationHash: string): Promise<PropertyGraph | null> {
   ...
   */
  async getOrComputePropertyGraph(
    derivation: CompatibilityDerivation,
    computeFn: () => Promise<PropertyGraph>
  ): Promise<{ hash: string; graph: PropertyGraph; cached: boolean }> {
    const result = await this.storeManager.getOrComputeResult(
      derivation,
      async () => {
   ...
   */
  async storeCompatibilityIssues(issues: any[]): Promise<string> {
   ...
   */
  async getCompatibilityIssues(derivationHash: string): Promise<any[] | null> {
    const propertyGraph = await this.getPropertyGraph(derivationHash);
    if (!propertyGraph) return null;
   ...
   */
  async queryByDerivationPattern(pattern: Partial<CompatibilityDerivation>): Promise<PropertyGraphStoreEntry[]> {
    const allHashes = await this.storeManager.listDerivations();
    const matchingEntries: PropertyGraphStoreEntry[] = [];

    for (const hash of allHashes) {
      const entry = await this.storeManager.getResult(hash);
      if (!entry || !entry.result.propertyGraph) continue;
   ...
   */
  async mergePropertyGraphs(derivationHashes: string[], mergeDerivation: CompatibilityDerivation): Promise<string> {
    // Load all graphs
    const graphs: PropertyGraph[] = [];
    ...
    const mergedGraph: PropertyGraph = {
      meta: {
   ...
   */
  async exportPropertyGraph(
    derivationHash: string,
    format: 'json' | 'jsonl' | 'csv' = 'json'
  ): Promise<string> {
    const graph = await this.getPropertyGraph(derivationHash);
    if (!graph) throw new Error(`Property graph not found: ${derivationHash}`);
    ...
    switch (format) {
      ...
      case 'csv':
        ...
        const csvLines: string[] = ['type,id,labels,properties'];
   ...
   */
  async diffPropertyGraphs(hash1: string, hash2: string): Promise<{
    added: { vertices: PropertyVertex[]; incidents: PropertyIncident[]; events: PropertyEvent[] };
    removed: { vertices: PropertyVertex[]; incidents: PropertyIncident[]; events: PropertyEvent[] };
    modified: { vertices: PropertyVertex[]; incidents: PropertyIncident[]; events: PropertyEvent[] };
  }> {
  ...
  private derivationMatchesPattern(derivation: CompatibilityDerivation, pattern: Partial<CompatibilityDerivation>): boolean {
    ...
    if (pattern.react && derivation.react !== pattern.react) return false;
    if (pattern.node && derivation.node !== pattern.node) return false;
    if (pattern.packageManager && derivation.packageManager !== pattern.packageManager) return false;
...
export class PropertyGraphStore {
  private manager: PropertyGraphStoreManager;
   ...
   */
  async storeCompatibilityData(issues: any[]): Promise<string> {
   ...
   */
  async getCompatibilityData(hash: string): Promise<any[] | null> {
   ...
   * Query stored graphs
   */
  async queryStoredGraphs(pattern: Partial<CompatibilityDerivation>): Promise<PropertyGraphStoreEntry[]> {
   ...
   */
  async exportGraph(hash: string, format: 'json' | 'jsonl' | 'csv' = 'json'): Promise<string> {
